...
export class TreasuryAgent {
  ...
  /** Initialize wallet, lending, swap, and LLM */
  async init () {
    this.wallet = createWalletFromEnv()
    this.walletSource = 'ethers'

    // Aave V3 lending
    if (process.env.AAVE_POOL) {
    ...
    await this.refresh()
    this.log('init', `Agent initialized — wallet ${this.wallet.address}, swap: ${this.swapProvider}`, {
  ...
  getSnapshot () {
    return {
      ...
      walletSource: this.walletSource || 'ethers',
      active: this.active,
      paused: this.paused,
  ...
  log (type, message, data = {}) {
    ...
    console.log(`[dotagent] [${type}] ${message}`)
  }
  ...
  logError (context, error, extra = {}) {
    ...
    console.error(`[dotagent] [ERROR:${context}] ${error.message}`)
  }
}
